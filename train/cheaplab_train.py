...
def train(model, opt, obj,
    ...
    for i in range(starting_epoch, epochs):
        ...
        if (i > 0) and (i % 5 == 0) and bucket_name and s3_prefix and not no_checkpoints:
            torch.save(model.state_dict(), 'cheaplab.pth')
            s3 = boto3.client('s3')
            s3.upload_file('cheaplab.pth', bucket_name,
                           '{}/{}/cheaplab_checkpoint_{}.pth'.format(s3_prefix, arg_hash, i))
            del s3
...
class CheapLab(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.numerator = torch.nn.Conv2d(
            in_channels, in_channels, kernel_size=7, padding=3, bias=True)
        self.denomenator = torch.nn.Conv2d(
            in_channels, in_channels, kernel_size=7, padding=3, bias=True)
        self.index = torch.nn.Conv2d(
            in_channels, out_channels, kernel_size=7, padding=3, bias=True)
        numerator_weights = np.zeros((in_channels, in_channels, 7, 7), dtype=np.float32)
        numerator_weights[:][1][:][:] = +0.1
        numerator_weights[:][4][:][:] = -0.1
        self.numerator.weight.data = torch.from_numpy(numerator_weights)
        denomenator_weights = np.zeros((in_channels, in_channels, 7, 7), dtype=np.float32)
        denomenator_weights[:][1][:][:] = +0.1
        denomenator_weights[:][4][:][:] = +0.1
        self.denomenator.weight.data = torch.from_numpy(denomenator_weights)

    def forward(self, x):
        numerator = self.numerator(x)
        denomenator = self.denomenator(x)
        quotient = torch.div(numerator, denomenator)
        index = self.index(quotient)
        return index


def make_model(band_count, class_count=2):
    return CheapLab(band_count, class_count)
...
if __name__ == '__main__':
    ...
    hashed_args.script = 'cheaplab_train.py'
    ...
    s3.upload_file('/tmp/args.txt', args.s3_bucket,
                   '{}/{}/cheaplab_training_args.txt'.format(args.s3_prefix, arg_hash))
    del s3
    ...
    if args.start_from is None:
        for pth in get_matching_s3_keys(
                ...
                suffix='pth'):
            m1 = re.match('.*cheaplab_(\d+).pth$', pth)
            m2 = re.match('.*cheaplab_checkpoint_(\d+).pth', pth)
            if m1:
    ...
    if complete_thru < 3:
        ...
        for p in cheaplab.parameters():
            p.requires_grad = True

        ps = []
        for n, p in cheaplab.named_parameters():
            if p.requires_grad == True:
                ps.append(p)
            else:
                p.grad = None
        opt = torch.optim.SGD(ps, lr=args.learning_rate, momentum=0.9)

        with rio.open('/tmp/mul.tif') as raster_ds, rio.open('/tmp/mask.tif') as mask_ds:
            train(cheaplab, opt, obj, steps_per_epoch, args.epochs, batch_size,
                  raster_ds, mask_ds, width, height, window_size, device,
                  args.bands, args.label_map, args.label_nd, args.img_nd, args.s3_bucket, args.s3_prefix, arg_hash,
                  no_checkpoints=False)

        print('\t UPLOADING')

        torch.save(cheaplab.state_dict(), 'cheaplab.pth')
        s3 = boto3.client('s3')
        s3.upload_file('cheaplab.pth', args.s3_bucket,
                       '{}/{}/cheaplab.pth'.format(args.s3_prefix, arg_hash))
        del s3
    ...
    if complete_thru == 4:
        ...
        s3 = boto3.client('s3')
        s3.download_file(args.s3_bucket, current_pth, 'cheaplab.pth')
        cheaplab = make_model(
            len(args.bands),
            len(args.weights)
        ).to(device)
        cheaplab.load_state_dict(torch.load('cheaplab.pth'))
        del s3
        ...
        with rio.open('/tmp/mul.tif') as raster_ds, rio.open('/tmp/mask.tif') as mask_ds:
            train(cheaplab, opt, obj, steps_per_epoch, args.epochs4, batch_size,
                  raster_ds, mask_ds, width, height, window_size, device,
                  args.bands, args.label_map, args.label_nd, args.img_nd, args.s3_bucket, args.s3_prefix, arg_hash,
