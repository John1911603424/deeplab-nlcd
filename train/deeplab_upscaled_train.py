...
def training_cli_parser():
    ...
    parser.add_argument('--learning-rate2',
                        # https://arxiv.org/abs/1206.5533
                        help='float (probably between 10^-6 and 1) to tune SGD',
                        default=os.environ.get('LEARNING_RATE_2', 0.01),
                        type=float)
...
def chainable_forward(self, x, **kwargs):
...
def make_upscaling_model(doublings, model):
    models = [model]
    for i in range(doublings):
        models.append(torch.nn.ConvTranspose2d(
            2, 2, kernel_size=6, stride=2, padding=2))
    deeplab = torch.nn.Sequential(*models)
    return deeplab
...
if __name__ == '__main__':
    ...
    hashed_args.script = 'deeplab_upscale_train.py'
    ...
    torchvision.models.segmentation.deeplabv3.DeepLabV3.forward = chainable_forward
    ...
    if complete_thru == 2:
        ...
        deeplab = make_upscaling_model(doublings, deeplab).to(device)
        deeplab.load_state_dict(torch.load('deeplab.pth'))
        del s3
        print('\t\t SUCCESSFULLY RESTARTED {}'.format(pth))
    elif complete_thru < 2:
        print('\t TRAINING UPSCALERS')
        ...
        for p in deeplab.parameters():
            p.requires_grad = False
        for i in range(doublings):
            for p in deeplab[i+1].parameters():
                p.requires_grad = True
