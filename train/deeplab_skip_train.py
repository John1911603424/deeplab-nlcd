...
class DeepLabSkip(torch.nn.Module):
    def __init__(self, in_channels, input_stride, out_channels):
        super(DeepLabSkip, self).__init__()
        deeplab = torchvision.models.segmentation.deeplabv3_resnet101(
            pretrained=True)
        deeplab.classifier[4] = torch.nn.Conv2d(
            256, out_channels, kernel_size=7, stride=1, dilation=1)
        deeplab.aux_classifier[4] = torch.nn.Conv2d(
            256, out_channels, kernel_size=7, stride=1, dilation=1)
        deeplab.backbone.conv1 = torch.nn.Conv2d(
            in_channels, 64, kernel_size=7, stride=input_stride, dilation=1, padding=(3, 3), bias=False)
        self.deeplab = deeplab
        self.mix = self.numerator = torch.nn.Conv2d(
            in_channels + out_channels, out_channels, kernel_size=7, padding=3, bias=True)

    def forward(self, x):
        y = self.deeplab(x)
        x = torch.cat([x, y['out']], dim=1)
        x = self.mix(x)
        return x


def make_model(device, band_count, input_stride=1, class_count=2):
    return DeepLabSkip(band_count, input_stride, class_count)
...
if __name__ == '__main__':
    ...
    hashed_args.script = 'deeplab_skip_train.py'
    ...
    if complete_thru == 0:
        ...
        last_class = deeplab.deeplab.classifier[4]
        last_class_aux = deeplab.deeplab.aux_classifier[4]
        input_filters = deeplab.deeplab.backbone.conv1
        mix = deeplab.mix
